...
ECHARM_SiEnhancement::ECHARM_SiEnhancement(double length){
    ...
    fileIn.open("Si110pot.txt");
    std::cout << "Si110pot.txt" << std::endl;
    if (fileIn.good()){
        fileIn.close();
        pot_intrp->ReadVecFromFile("Si110pot.txt");
    }
    else{
        ECHARM_EC_rec* pot = new ECHARM_EC_rec_pot_ax(fCrystal,128,128);
        pot_intrp->Store(pot);
        pot_intrp->PrintVecToFile("Si110pot.txt");
    }
    
    
    fileIn.open("Si110atd.txt");
    std::cout << "Si110atd.txt" << std::endl;
    if (fileIn.good()){
        fileIn.close();
        atd_intrp->ReadVecFromFile("Si110atd.txt");
    }
    else{
        ECHARM_EC_rec* atd = new ECHARM_EC_rec_atd_ax(fCrystal,128,128);
        atd_intrp->Store(atd);
        atd_intrp->PrintVecToFile("Si110atd.txt");
    }
    
    
    fileIn.open("Si110eld.txt");
    std::cout << "Si110eld.txt" << std::endl;
    if (fileIn.good()){
        fileIn.close();
        eld_intrp->ReadVecFromFile("Si110eld.txt");
    }
    else{
        ECHARM_EC_rec* eld = new ECHARM_EC_rec_eld_ax(fCrystal,128,128);
        eld_intrp->Store(eld);
        eld_intrp->PrintVecToFile("Si110eld.txt");
    }
    
    
    fileIn.open("Si110efx.txt");
    std::cout << "Si110efx.txt" << std::endl;
    if (fileIn.good()){
        fileIn.close();
        efx_intrp->ReadVecFromFile("Si110efx.txt");
    }
    else{
        ECHARM_EC_rec* efx = new ECHARM_EC_rec_efx_ax(fCrystal,128,128);
        efx_intrp->Store(efx);
        efx_intrp->PrintVecToFile("Si110efx.txt");
    }
    
    fileIn.open("Si110efy.txt");
    std::cout << "Si110efy.txt" << std::endl;
    if (fileIn.good()){
        fileIn.close();
        efy_intrp->ReadVecFromFile("Si110efy.txt");
    }
    else{
        ECHARM_EC_rec* efy = new ECHARM_EC_rec_efy_ax(fCrystal,128,128);
        efy_intrp->Store(efy);
        efy_intrp->PrintVecToFile("Si110efy.txt");
    }
