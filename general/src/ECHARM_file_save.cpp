...
void ECHARM_file_save::UpdateInfoTraj(ECHARM_info_save* info,int i0){
        ...
        partNum = info->GetPartNum();
        dchTimes = info->GetDechTimesTot().at(i0);
        chTimes = info->GetChTimesTot().at(i0);
        chIn = info->GetChInitialTot().at(i0);
}
...
void ECHARM_file_save::Save(ECHARM_info_save* info){
    ...
    for(unsigned int i0=0;i0<info->GetAtD().size();i0++){
        ...
        fFileOutTraj << partNum << " " << i0 << " " << chIn << " " << partStep.posz << " " << partStep.posx << " " << partStep.posy << " " << partStep.angx << " " << partStep.angy << " " << pot/eV  << " " << kin/eV << " " << atd << " " << eld << std::endl;
   }
